    ...
    actions
    {
        area(processing)
        {
            group("&Functions")
            {
                ...
                action("Update Cost Centers")
                {
                    Visible = true;
                    ApplicationArea = Basic, Suite;
                    Image = UpdateCostCenter;
                    Promoted = true;
                    PromotedCategory = Process;
                    RunObject = report 52178546;

                    // trigger OnAction()
                    // var
                    // PostedSrn : Record "PROC-Store Requistion Header";
                    // GLPost : Record "G/L Entry";
                    // begin
                    //     Rec.Reset();
                    //     Rec.SetRange(Status, Rec.Status::Posted);
                    //     rec.SetRange("No.", GLPost."Document No.");
            
                    //     Rec.SetFilter("Shortcut Dimension 2 Code", GLPost."Global Dimension 2 Code");
                    //     if Rec.FindSet() then
                    //         repeat
                    //             GLPost."Global Dimension 2 Code" := PostedSrn."Shortcut Dimension 2 Code";
                    //             Rec.Modify();
                    //         until Rec.Next() = 0;
