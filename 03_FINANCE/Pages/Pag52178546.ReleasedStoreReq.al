page 52178546 "Released Store Req"

{
    CardPageID = "PROC-Store Req. Header (B)";
    PageType = List;
    SourceTable = "PROC-Store Requistion Header";
    SourceTableView = where(status = const(Open), status = filter(<>"Pending Approval"), status = filter(<>released), status = filter(<> Posted), status = filter(<> Cancelled));
