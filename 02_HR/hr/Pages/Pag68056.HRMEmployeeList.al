page 68056 "HRM-Employee-List"
{
    ...
    SourceTableView = SORTING("No.") ORDER(Ascending) where(Status = filter(active), "Posting Group" = filter('SALARY'));// "Payroll Posting Group" = filter('PAYROLL'));
            ...
            action(ViewhorPayslip)
            {
                ...
                begin
                    ...
                    if prtransactions.Find('-') then
                        REPORT.Run(Report::"PRL-Payslips V 1.1.1", true, false, prtransactions);
                end;
            }
        ...
        area(processing)
        {
            action(ProcessPayroll)
            {
                ...
                begin
                    ...
                    objPeriod.Reset;
                    objPeriod.SetRange(objPeriod.Closed, false);
                    // objPeriod.SetFilter("Payroll Code", '%1', 'SALARY');
